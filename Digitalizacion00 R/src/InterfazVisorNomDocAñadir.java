...
public class InterfazVisorNomDocAñadir extends JDialog{

	/**
	 * @param args
	 */
	JButton elegirJB = new JButton("Elegir Imagen");
...
//	JLabel etiqueta = new JLabel("   Nombre del Documento   ");
	...
	InterfazVisorNomDocAñadir(){
		setTitle("Visor de Documentos");
		setModal(true);

		elegirJB.setBackground(Color.pink);
		elegirJB.addActionListener(new ActionListener(){

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				System.out.println(rutaImagenSeleccionada + " otra vez");
	    ...
	    comboDocumentos.addActionListener(new java.awt.event.ActionListener() {
	            public void actionPerformed(java.awt.event.ActionEvent evt) {
	            	String[] documentos;
	            	documentos = InicioIanus.excel.getDocServicioVisor(comboDocumentos.getSelectedItem().toString());
				...
				private ArrayList<String> getRutaJpg() {
					...
					for(int i=1;i<numFilas;i++){
						if(InicioIanus.excel.tablaVisor[i][1].toString().equalsIgnoreCase(comboDocumentos.getSelectedItem().toString())){
		...
		panelSuperior.add(elegirJB,BorderLayout.EAST);
	...
	public void EventoMouseClicked(final JTable tabla){
		tabla.addMouseListener(new MouseAdapter(){
			...
			public void mousePressed(MouseEvent e){
				...
				System.out.println(rutaImagenSeleccionada + "hola");
				rutaImagenSeleccionada = rutaJpgs.get(fil*3 + column);
				System.out.println(rutaImagenSeleccionada + " adios");
			}
...
DefaultComboBoxModel nombresDocumentosDCBM = new DefaultComboBoxModel();
		...
		for(int i=1;i<numFilas;i++){
				nomDocumentos.add(InicioIanus.excel.tablaVisor[i][1].toString());
		}
