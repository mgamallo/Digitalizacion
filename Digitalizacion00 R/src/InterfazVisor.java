...
*/	
	...
	InterfazVisor(){
	    ...
	    comboServicios.addActionListener(new java.awt.event.ActionListener() {
	            public void actionPerformed(java.awt.event.ActionEvent evt) {
	            	String[] docServicios;
	            	docServicios = InicioIanus.excel.getDocServicioVisor(comboServicios.getSelectedItem().toString());
				...
				private ArrayList<String> getDocumentosJpg() {
					...
					for(int i=1;i<numFilas;i++){
						if(InicioIanus.excel.tablaVisor[i][2].toString().contains(comboServicios.getSelectedItem().toString())){
							listaDocumentos.add(InicioIanus.excel.tablaVisor[i][1].toString());
						}
					}
	...
	DefaultComboBoxModel listaServicios(){
		...
		int numFilas = InicioIanus.excel.tablaVisor.length;
//				InicioIanus.excel.tablaVisor[j][1].toString().contains(docServicios[i])
		servicios.addElement("Selecciona Servicio");
		servicios.addElement(InicioIanus.excel.tablaVisor[1][2].toString());
		for(int i=2;i<numFilas;i++){
			if(!InicioIanus.excel.tablaVisor[i][2].toString().contains(InicioIanus.excel.tablaVisor[i-1][2].toString())){
				servicios.addElement(InicioIanus.excel.tablaVisor[i][2].toString());
			}
		}
